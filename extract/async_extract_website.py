...
async def entity_fishing(input_path, output_path):
    url = 'http://localhost:8090/service/website'
    query = {}
...
async def async_extract_all(datapath, outputpath,textfield, companyfield):
    ...
    for filepath in sorted(glob(raw_dir, recursive=True), reverse=True):
        ...
        await entity_fishing(input_path = filepath,output_path = os.path.join(output_dir,current_ds,filename+'-extracted.json'))
